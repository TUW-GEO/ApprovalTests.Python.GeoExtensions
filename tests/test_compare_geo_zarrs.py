...
from pytest_approvaltests_geo.compare_geo_zarr import CompareGeoZarr
...
def test_compare_zarrs_tiffs_applies_scrubbers_to_tags(tmp_path):
    ...
    scrubbing_comparator = CompareGeoZarr(make_scrubber_recurse(date_scrubber))
...
def test_compare_geo_zarrs_floats_with_tolerances(tmp_path):
    tolerant_comparator = CompareGeoZarr(float_tolerance=Tolerance(rel=0.008, abs=0.0021))
